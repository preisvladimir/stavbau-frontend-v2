...
// (volitelné – pokud budeš mapovat API chyby do polí)
...
export const Form: React.FC<FormProps> = ({
  ...
  // Volitelně: mapování serverError do formuláře (necháváme vypnuto jako v TeamForm)
  React.useEffect(() => {
    if (!serverError) return;
    // Příklad: setError('name', { type: 'server', message: serverError });
    // Případně:
    // const p = toApiProblem(serverError);
  ...
  const onAddressPick = (addr: AddressSuggestion) => {
    ...
    const dto: AddressDto = {
      ...
      orientationNumber: nn((addr as any).orientationNumber ?? addr.houseNumber), // ✅ preferujeme orientationNumber, fallback na houseNumber
