...
import { UserPlus, Pencil, Trash2, X } from '@/components/icons';
...
export default function TeamPage() {
  const { setFab } = useFab();
  const i18nNamespaces = React.useMemo(() => ['team', 'common'], []);
  ...
  // FAB
  React.useEffect(() => {
    setFab({ label: t('list.actions.add'), onClick: () => openNew(), icon: <Plus className="h-6 w-6" /> });
    return () => setFab(null);
  }, [setFab]);
  ...
  return (
    <div className="p-4">
      <div className={cn(sbContainer)}>
        ...
        {/* Loading & Error state (zviditelní použití `error`) */}
        {loading && <div className="mb-2">{t('loading', { defaultValue: 'Načítám…' })}</div>}
        {!loading && error && (
        ...
        {/* Edit form – načítá si detail sama (prefill z listu) */}
        <TeamFormDrawer
          mode="edit"
          i18nNamespaces={i18nNamespaces}
          open={!!isEdit}
          ...
          defaultValues={{
            ...
            companyRole: items.find((i) => i.id === params.id)?.role ?? null,
            sendInvite: false,
          }}
        />
      </div>
